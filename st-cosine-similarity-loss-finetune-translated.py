...
def load_data(input_data, input_dict, max_len=256):
  ...
  for i in range(len(input_data)):
    ...
    folder2 = f2[-2:]
    try:
      d1 = input_dict[f1+".json"]
      d2 = input_dict[f2+".json"]
...
eval_D = None
train_D = None
with open("./translated_articles.json", 'r+') as f:
    train_D = json.load(f)
with open("./translated_eval_data.json", 'r+') as f:
    eval_D = json.load(f)
train_dict = {}
for i in range(len(train_D)):
    train_dict[train_D[i]['filename']]=train_D[i]
eval_dict = {}
for i in range(len(eval_D)):
    eval_dict[eval_D[i]['filename']]=eval_D[i]
...
#Train Set
train_inputs1, train_inputs2, train_masks1, train_masks2, train_targets = load_data(train_data, train_dict, max_len=128)
train_dataloader = create_dataloaders(train_inputs1, train_inputs2, train_masks1, train_masks2, train_targets, batch_size)

#Val Set
val_inputs1, val_inputs2, val_masks1, val_masks2, val_targets = load_data(val_data, train_dict, max_len=128)
val_dataloader = create_dataloaders(val_inputs1, val_inputs2, val_masks1, val_masks2, val_targets, batch_size)

#Test Set
test_inputs1, test_inputs2, test_masks1, test_masks2, test_targets = load_data(test_data, eval_dict, max_len=128)
